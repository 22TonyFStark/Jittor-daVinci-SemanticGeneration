...
class Attention(jittor.Module):
    def __init__(self, ch, use_sn):
        ...
        if use_sn:
            ...
            self.theta = equal_lr(self.theta)
            self.phi = equal_lr(self.phi)
            self.g = equal_lr(self.g)
            self.o = equal_lr(self.o)
    ...
    def execute(self, x, y=None):
        ...
        #print('x',x.shape)
        theta = self.theta(x)
        #phi = jittor.nn.max_pool2d(self.phi(x), 2)
        #g = jittor.nn.max_pool2d(self.g(x), 2)    
        phi = nn.interpolate(self.phi(x), scale_factor=0.5, mode='bilinear')
        g = nn.interpolate(self.g(x), scale_factor=0.5, mode='bilinear')
        # Perform reshapes
