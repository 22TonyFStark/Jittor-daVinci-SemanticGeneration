...
def max_diameter(x, y):
    mins = jittor.misc.stack((x.min(dim=0), y.min(dim=0))).min(dim=0)
    maxs = jittor.misc.stack((x.max(dim=0), y.max(dim=0))).max(dim=0)
...
class UnbalancedWeight(nn.Module):
...
def sinkhorn_cost(ε, ρ, α, β, a_x, b_y, a_y, b_x, batch=False, debias=True, potentials=False):
    ...
    if debias:  # UNBIASED Sinkhorn divergence, S_ε(α,β) = OT_ε(α,β) - .5*OT_ε(α,α) - .5*OT_ε(β,β)
        if ρ is None:
            ...
            return b_x - a_x, a_y - b_y, scal(α, UnbalancedWeight(ε, ρ)( (-a_x/ρ).exp() - (-b_x/ρ).exp()), batch=batch) \
        ...
        if ρ is None:
            ...
            return b_x, a_y, scal(α, UnbalancedWeight(ε, ρ)(1 - (-b_x/ρ).exp()), batch=batch) \
...
def sinkhorn_loop( softmin, α_logs, β_logs, C_xxs, C_yys, C_xys, C_yxs, ε_s, ρ, 
    ...
    with jittor.no_grad():
        ...
        for i, ε in enumerate(ε_s):  # ε-scaling descent -----------------------
            ...
            """
            print(at_y.flatten()[:3])
            print('*'*30)
            print(bt_x.flatten()[:3])
            print('*'*30)
            print(λ)
            print('*'*30)
            print(α_log.flatten()[:3],β_log.flatten()[:3],ε)
            print('*'*30)
            print(C_yx.flatten()[:3],C_xy.flatten()[:3])
            print('*'*30)
            print(i,a_y.flatten()[:3], b_x.flatten()[:3])
            print('*'*30)
            """
    ...
    #print('λ',λ, 'ε',ε, 'α_log',α_log[:2], 'β_log',β_log[:2],'b_x',b_x[:2],'a_y',a_y[:2])
