...
class SamplesLoss(jittor.Module):
    ...
    def generate_weights(self, x):
        if x.ndim == 2:  # 
            ...
            ret = jittor.ones(N).type_as(x) / N
            ret.stop_grad()
            return ret
        elif x.ndim == 3:
            B, N, _ = x.shape
            ret = jittor.ones(B,N).type_as(x) / N
            ret.stop_grad()
            return ret
        else:
    ...
    def check_shapes(self, l_x, α, x, l_y, β, y):
        ...
        if α.ndim != β.ndim: raise ValueError("Input weights 'α' and 'β' should have the same number of dimensions.")

        if x.ndim != y.ndim: raise ValueError("Input samples 'x' and 'y' should have the same number of dimensions.")
        ...
        elif x.ndim == 3:  # batch computation ---------------------------------------------------------
...
if __name__ == '__main__':
    ...
    np.random.seed(1)
    a = np.random.randn([6,4096])
    np.random.seed(2)
    b = np.random.randn([6,4096, 2304])
    np.random.seed(3)
    c = np.random.randn([6,4096])
    np.random.seed(4)
    d = np.random.randn([6,4096, 2304])
    F,G = uot(a,b,c,d)
    print(F.flatten()[::100])
    print(G.flatten()[::100])
    print('finish')