...
from models.networks.correspondence import VGG19_feature_color
    ...
    def loss(self, input, target_is_real, for_discriminator=True):
        if self.gan_mode == 'original':  # cross entropy loss
            ...
            if for_discriminator:
                if target_is_real:
                    minval = jittor.minimum(input - 1, self.get_zero_tensor(input))
                    loss = -jittor.mean(minval)
                else:
                    minval = jittor.minimum(-input - 1, self.get_zero_tensor(input))
                    loss = -jittor.mean(minval)
            else:
                assert target_is_real, "The generator's hinge loss must be aiming for real"
                loss = -jittor.mean(input)
            return loss
        else:
            # wgan
            if target_is_real:
    ...
    def __call__(self, input, target_is_real, for_discriminator=True):
        ...
        if isinstance(input, list):
            ...
            for pred_i in input:
                ...
                new_loss = jittor.mean(loss_tensor.view(bs, -1), dim=1)
                loss += new_loss
...
class VGGLoss(nn.Module):
    def __init__(self, gpu_ids, vgg_normal_correct=False):
        ...
        if vgg_normal_correct:
            self.vgg = VGG19_feature_color(vgg_normal_correct=True)
        else:
            self.vgg = VGG19()
